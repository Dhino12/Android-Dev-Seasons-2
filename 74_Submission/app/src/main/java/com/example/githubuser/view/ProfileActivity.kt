...
import com.example.githubuser.model.SectionsPagerAdapter
...
//        favoriteItems = intent.getParcelableExtra(EXTRA_FAVORITE)
//        coverProfile.setImageResource(users.colors)
        ...
        if(view?.id == R.id.fab_add){
            ...
            if(it?.userName != null && idFromDB == null){
                ...
                if(isEdit){
                    ...
                    val result = favoriteHelper.update(favoriteItems?.id.toString(),values).toLong()
                    if(result > 0){
                        setResult(RESULT_UPDATE,intent)
                        finish()
                    }else{
                        Toast.makeText(this,"Gagal Mengupdate Favorite",Toast.LENGTH_LONG).show()
                    }
                }else{
